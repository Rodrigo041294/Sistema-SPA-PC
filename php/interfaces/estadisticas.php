                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="card card-calendar">
                            <div class="col-md-3">
                                <form action="../request/generar_reporte.php" method="POST">
                            ...
                            <div class="col-md-4">
                                <div class="form-group label-floating ">
                                    ...
                                    <select class="form-control" name="tipo">
                                        <option>Control de Servicios</option>
...
<script>
    ...
    series: [{
        ...
        data: [
            <?php
            $finalizados_equ = "SELECT id FROM equipos_revision WHERE id_salida != ''";
            $resfin = mysqli_query($mysqli, $finalizados_equ);
            $finalizadosnum = mysqli_num_rows($resfin);

            $finalizados_serv = "SELECT id FROM servicios WHERE estatus = '1'";
            $resfin2 = mysqli_query($mysqli, $finalizados_serv);
            $finalizadosnum2 = mysqli_num_rows($resfin2);

            $totalfonalizados = $finalizadosnum + $finalizadosnum2;



            $totales = "SELECT id FROM equipos_revision";
            $restot = mysqli_query($mysqli, $totales);
            $totaleseq = mysqli_num_rows($restot);

            $totales2 = "SELECT id FROM servicios";
            $restot2 = mysqli_query($mysqli, $totales2);
            $totaleser = mysqli_num_rows($restot2);

            $totales = $totaleseq + $totaleser;


            $pendientes = $totales - $totalfonalizados;
            echo "['finalizados',".$totalfonalizados."],";
...
Highcharts.chart('container1', {
    ...
    series: [{
        ...
        data: [
        <?php
        $contar = "SELECT u.nombre as nombre, COUNT(e.id) as total FROM equipos_revision e INNER JOIN usuarios u ON e.asignado = u.id GROUP BY u.nombre";
        $rescontar = mysqli_query($mysqli, $contar);
        if($rescontar)
        {
            while($datosnum = mysqli_fetch_assoc($rescontar))
            {
                echo "{name: '".$datosnum['nombre']."', y:".$datosnum['total'].",sliced: true,selected: true},";
            }
        }
        ?>
